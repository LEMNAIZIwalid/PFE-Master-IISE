...
#ifndef _TFT_ESPRITE_H_
#define _TFT_ESPRITE_H_

#include "Arduino.h"
#include "TFT_eSPI.h"

/***************************************************************************************
// La classe suivante crée des Sprites en RAM. Les graphiques sont dessinés dans le Sprite
// puis rendus rapidement sur l'écran TFT. La classe hérite de TFT_eSPI.
***************************************************************************************/
...
class TFT_eSprite : public TFT_eSPI {
 ...
 public:
  ...
  void     drawPixel(int32_t x, int32_t y, uint32_t color);
  void     drawChar(int32_t x, int32_t y, uint16_t c, uint32_t color, uint32_t bg, uint8_t size); // Corrigé ;
  void     fillSprite(uint32_t color); // Corrigé ;
  
  void     setWindow(int32_t x0, int32_t y0, int32_t x1, int32_t y1); // Corrigé ;
  void     pushColor(uint16_t color); // Corrigé ;
  void     pushColor(uint16_t color, uint32_t len); // Corrigé ;
  void     writeColor(uint16_t color); // Corrigé ;

  void     setScrollRect(int32_t x, int32_t y, int32_t w, int32_t h, uint16_t color = TFT_BLACK); // Corrigé ;
  void     scroll(int16_t dx, int16_t dy = 0); // Corrigé ;

  void     drawLine(int32_t x0, int32_t y0, int32_t x1, int32_t y1, uint32_t color); // Corrigé ;
  void     drawFastVLine(int32_t x, int32_t y, int32_t h, uint32_t color); // Corrigé ;
  void     drawFastHLine(int32_t x, int32_t y, int32_t w, uint32_t color); // Corrigé ;
  ...
  int16_t  drawChar(uint16_t uniCode, int32_t x, int32_t y, uint8_t font); // Corrigé ;
  int16_t  drawChar(uint16_t uniCode, int32_t x, int32_t y); // Corrigé ;

  int16_t  width(void); // Corrigé ;
  int16_t  height(void); // Corrigé ;
 ...
 protected:
  ...
  int32_t  _dwidth, _dheight; 
  int32_t  _bitwidth;         
};

#endif // _TFT_ESPRITE_H_
